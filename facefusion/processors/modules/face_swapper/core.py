...
from facefusion.face_selector import select_faces, sort_faces_by_order,sort_and_filter_faces
...
def process_frame(inputs : FaceSwapperInputs) -> ProcessorOutputs:
	...
	if mode == 'reference':
		return process_frame_reference(inputs)


def init_reference_faces():

	if state_manager.get_item("reference_faces"):
		return

	reference_faces = []
	source_faces = []

	target_face_paths = state_manager.get_item("target_face_paths")
	source_paths = state_manager.get_item("source_paths")

	if not target_face_paths or not source_paths:
		return

	for target_path, source_path in zip(target_face_paths, source_paths):

		target_img = read_image(target_path)
		target_face_temp = get_many_faces([target_img])
		target_face = target_face_temp[0] if isinstance(target_face_temp, (list, tuple)) and len(target_face_temp) > 0 else None

		source_img = read_image(source_path)
		source_face_temp = get_many_faces([source_img])
		source_face = source_face_temp[0] if isinstance(source_face_temp, (list, tuple)) and len(source_face_temp) > 0 else None

		if target_face is not None and source_face is not None:
			reference_faces.append(target_face)
			source_faces.append(source_face)

	state_manager.set_item("reference_faces", reference_faces)
	state_manager.set_item("source_faces", source_faces)


def process_frame_reference(inputs: FaceSwapperInputs) -> ProcessorOutputs:

	init_reference_faces()
	...
	matched_map = state_manager.get_item("matched_reference_map")
	source_faces = state_manager.get_item("source_faces")

	if not matched_map or not source_faces:
		return temp_vision_frame, temp_vision_mask

	for target_face in target_faces:
		idx = matched_map.get(id(target_face))
		if idx is None:
			continue
		source_face = source_faces[idx]
		print("swap using source index:", idx)
		target_face = scale_face(target_face, target_vision_frame, temp_vision_frame)
		temp_vision_frame = swap_face(
			source_face,
			target_face,
			temp_vision_frame
		)
