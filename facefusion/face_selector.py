...
def select_faces(reference_vision_frame: VisionFrame, target_vision_frame: VisionFrame) -> List[Face]:
	...
	# reference 模式：根据 reference face 匹配
	if state_manager.get_item('face_selector_mode') == 'reference':
		target_face_paths = state_manager.get_item("target_face_paths")
		print("target_face_paths:", len(target_face_paths))
		if target_face_paths:
			reference_faces = []

			# 加载 reference faces
			for path in target_face_paths:
				img = read_image(path)
				faces = get_many_faces([img])
				if faces:
					reference_faces.append(faces[0])  # 每张图只有一个人脸
			...
			threshold = state_manager.get_item("reference_face_distance")

			for i, reference_face in enumerate(reference_faces):
				best_face = None
				best_distance = 2.0
				for target_face in target_faces:
					dist = calculate_face_distance(target_face, reference_face)
					if dist < best_distance:
						best_distance = dist
						best_face = target_face

				print("reference", i, "best_distance:", best_distance)
				if best_face and best_distance <= threshold:
					matched_faces.append(best_face)
					matched_map[id(best_face)] = i
			state_manager.set_item("matched_reference_map", matched_map)
...
def calculate_face_distance2(face : Face, reference_face : Face) -> float:

	if hasattr(face, "normed_embedding") and hasattr(reference_face, "normed_embedding"):
		return 1 - numpy.dot(face.normed_embedding, reference_face.normed_embedding)

	if hasattr(face, "embedding") and hasattr(reference_face, "embedding"):
		diff = face.embedding - reference_face.embedding
		return float(numpy.sqrt((diff * diff).sum()))
